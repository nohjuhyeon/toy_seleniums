...
def mongodb(client,DB,collection_name):                         # mongodb 연결
    ...
    return collection       
def selectCourts(browser,collection):                                                                                                           # 리뷰 데이터베이스에 전송
    from selenium.webdriver.common.by import By
    ...
    return item_list
    # 브라우저 종료
def page_shift(browser,item_list):  
        from selenium.webdriver.common.by import By
        for i in range(5):
            try:
                review_upload= item_list                             # 리뷰 데이터 베이스에 전송
...
if __name__ == "__main__":
    try: 
        ...
        review_upload = selectCourts(browser, collection)
        page_shift(browser,review_upload)
        pass
    except:
